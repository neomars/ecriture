# Écriture — Developer Guide

> Technical documentation (English): building from source, architecture, local AI and GPU acceleration. For the user-facing presentation and installation from the releases, see [README.md](README.md).
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/neomars/ecriture.git
   cd ecriture
   ```
...
- **`ecriture-core`** — framework-agnostic business logic (no Tauri
  ...
  txt/docx/pdf/odt/epub/mobi, French and English synonym lookup (bundled `lexique.db` and
  `thesaurus_en.db`),
...
not silently faked):
- Synonym lookup has real data for French (`lexique.db`) and English
  (`thesaurus_en.db`, built from WordNet 3.0 - the data the Python app used
  through NLTK). Spanish and Russian (spaCy in the Python app) currently
  return an empty list rather than pretending to work.
...
Both files are generated by scripts in `ecriture/scripts/` and committed;
re-run a script only to change the data:

- `ecriture-core/resources/thesaurus_en.db` (+ `WORDNET_LICENSE.txt`, which
  must ship with it): `python3 scripts/build_thesaurus_en.py <wordnet dict dir>`,
  e.g. the `wordnet/` folder of NLTK's `corpora/wordnet.zip`.
- `ecriture-core/resources/default_projects/pride_and_prejudice.json`:
  `python3 scripts/build_pride_and_prejudice.py <Project Gutenberg 1342.txt>`;
  chapter summaries, characters, plot cards and notes live in the script.
